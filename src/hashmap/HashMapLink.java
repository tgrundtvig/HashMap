...
public class HashMapLink<K, V> implements Map<K, V> {
    ...
    public V put(K key, V value) {
        ...
        while (true) {
            if (cur == null) {
                Link<K, V> newLink = new Link(new MapEntry(key, value), null);//Link but null
                array[index] = newLink;
                ++size;
                expandArray();
                return null;
    ...
    public V remove(K key) {
        int index = getIndex(key);
        Link<K, V> head = array[index];
//        int count = 0; // we have to know what to delete

        if(head == null){
            return null;
        }
        
        if(head.getEntry().getKey().equals(key)){
            Link<K, V> removed = head;
            array[index] = head.getNext();
//            head.setNext(null); // is this needed?
            head = null;
        ...
        while (true) {
            ...
            } else if (cur.getNext().getEntry().getKey().equals(key)) {
                Link<K, V> toBeRemoved = cur.getNext();
                cur.setNext(toBeRemoved.getNext());
                toBeRemoved.setNext(null);
                return toBeRemoved.getEntry().getValue();
